...
def instruction_candidates(markdown)
  ...
  lines.each_with_object([]) do |line, list|
    ...
    next unless normalized.match?(/\b(MUST|SHOULD|should|must|Use|Do not|Don't|Never|Always|Run|Verify|Load|Create|Write|Review|Check|Avoid|Prefer|Inject|Register|Define|Extract|Compose|Scaffold)\b/)
...
def fallback_instructions(skill_name, description)
  [
    ...
    "Follow the skill-specific workflow and gates instead of giving generic Hanami advice.",
  ...
  instructions = candidates.map do |candidate|
    why =
      if candidate.match?(/\b(MUST|must|Never|Do not|Don't|HARD-GATE)\b/)
        "preference"
      elsif candidate.match?(/\b(Use|Run|Verify|Load|Create|Write|Review|Check|Inject|Register|Define|Extract|Compose|Scaffold)\b/)
        "new knowledge"
      else
        "reminder"
      end
...
def write_skill_eval(skill_name, skill_path)
      ...
      "instructions" => instructions.map do |instruction|
        {
          ...
          "relevant_when" => "When a user asks for Hanami work that should trigger #{skill_name}.",
  ...
  File.write(
    ...
    <<~MARKDOWN
      ...
      #{description.empty? ? "Use the repository's Hanami skill conventions to produce the requested result." : description}
  ...
  write_json(
    ...
    {
      "context" => "Checks whether the final artifact follows the #{skill_name} instructions from the published hanakai-yaku plugin.",
