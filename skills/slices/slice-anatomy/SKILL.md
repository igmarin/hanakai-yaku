...
|---|---|
| **routes-dsl** | Slices define their own routes. Master routes before creating slices. |
| **deps-mixin** | Cross-slice dependencies use `import`/`export` and are injected via `Deps`. |
| **slice-configuration** | Configure slice-level settings and providers after creating the slice. |
| **new-slice** (workflow) | Full workflow for creating and configuring a new slice. |
