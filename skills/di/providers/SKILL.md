...
|---|---|
| **deps-mixin** | Provided components are injected via `Deps[]`. Understand `Deps` before writing providers. |
| **settings** | Providers read configuration from `target[:settings]`. Define settings before writing providers. |
| **action-anatomy** | Actions inject provided services via `Deps[]`. |
| **rom-repositories** | The database provider registers ROM, which Repositories depend on. |
