...
|---|---|
| **rom-relations** | Repository wraps a Relation. Define the Relation first, then the Repository. |
| **rom-structs-entities** | Repository returns Entity objects. Define the Entity schema to match. |
| **action-anatomy** | Actions inject Repositories via `Deps` and pass data to Views. |
| **dry-monads/result-pattern** | Complex Repository operations return `Success`/`Failure` for explicit error handling. |
| **rom-specs** (testing) | Test Repository methods with in-memory ROM. |
