...
8. **Test Relations** with in-memory ROM setup (`skills/testing/rom-specs`).
...
|---|---|
| **sequel-migrations** | After any migration that changes the schema — verify Relation still matches |
| **rom-repositories** | When you need domain-level read/write operations that wrap Relations |
| **rom-structs-entities** | When defining the data structures returned by Relations |
| **rom-specs** (testing) | When writing tests for Relation query methods |
| **crud-resource-workflow** | Full end-to-end: Relation → Repository → Action → View → Test |
