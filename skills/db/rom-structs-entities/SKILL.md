...
|---|---|
| **rom-repositories** | Repository returns Entity objects. Define the Entity, then configure the Repository. |
| **rom-relations** | Relation schema drives Entity attributes. Keep them in sync. |
| **action-anatomy** | Actions receive Entities from Repositories and pass them to Views. |
| **view-objects** | Views receive Entities as exposures. Entities are the data contract between layers. |
| **sequel-migrations** | Schema changes require Entity updates. |
