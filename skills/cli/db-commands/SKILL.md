...
|---|---|
| **sequel-migrations** | Create migration files before running `hanami db migrate`. |
| **hanami-new** | Database commands are used after creating the app. |
| **rom-relations** | Verify Relations match the migrated schema. |
| **request-specs** | Test database setup may need `hanami db prepare`. |
