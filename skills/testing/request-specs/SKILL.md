...
|---|---|
| **action-anatomy** | Request specs test Actions. Understand Action structure first. |
| **action-params-validation** | Test invalid params in request specs (422 responses). |
| **rom-repositories** | Request specs exercise the full stack including Repositories. |
| **test-planning** | Decide when request specs are appropriate vs unit specs. |
| **tdd-workflow** | Follow the TDD workflow: write failing request spec → implement → verify pass. |
