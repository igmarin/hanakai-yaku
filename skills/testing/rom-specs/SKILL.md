...
|---|---|
| **rom-relations** | Test custom Relation query methods. |
| **rom-repositories** | Test Repository read/write operations. |
| **rom-structs-entities** | Assert on Entity attributes in ROM specs. |
| **test-planning** | Decide when ROM specs are needed vs request specs. |
| **tdd-workflow** | Follow the TDD workflow: write failing ROM spec → implement → verify pass. |
