...
|---|---|
| **action-anatomy** | Identify extraction candidates in Actions. |
| **deps-mixin** | Register and inject extracted components via `Deps`. |
| **dry-monads/result-pattern** | Service objects often return `Success`/`Failure`. |
| **rom-repositories** | Extract complex queries to Repository methods. |
| **test-planning** | Write characterization tests before refactoring. |
| **request-specs** | Verify full stack behavior after refactoring. |
