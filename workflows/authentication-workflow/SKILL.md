...
license: MIT
description: >
  Use when implementing authentication in Hanami 2.x. Chains deps-mixin,
  providers, action-anatomy, and action-halt-errors.
ecosystem_sources:
  - hanami/hanami
...
|---|---|---|
| 1. Setup DI | `deps-mixin` | Auth service injectable via `Deps` |
| 2. Register provider | `providers` | Auth service registered in container |
| 3. Create Actions | `action-anatomy` | Login, logout, protected endpoints |
| 4. Handle errors | `action-halt-errors` | 401/403 responses for auth failures |

---

## Core Process

1. **[Setup DI]** — Load skill: `deps-mixin`
...
3. **[Create Actions]** — Load skill: `action-anatomy`
...
4. **[Handle Errors]** — Load skill: `action-halt-errors`
...
|---|---|
| **deps-mixin** | Step 1: Inject auth service via `Deps`. |
| **providers** | Step 2: Register auth service in container. |
| **action-anatomy** | Step 3: Create login/logout/protected Actions. |
| **action-halt-errors** | Step 4: Handle 401/403 responses. |
