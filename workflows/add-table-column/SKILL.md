...
description: >
  ...
  sequel-migrations, rom-relations, rom-structs-entities, rom-repositories, and request-specs.
ecosystem_sources:
  - jeremyevans/sequel
  - rom-rb/rom
  - hanami/hanami
...
|---|---|---|
| 1. Generate migration | `sequel-migrations` | Migration file created |
| 2. Run migration | `cli/db-commands` | Migration applied successfully |
| 3. Update Relation | `rom-relations` | Relation schema includes new column |
| 4. Update Entity | `rom-structs-entities` | Entity has new attribute |
| 5. Update Repository | `rom-repositories` | Repository methods use new column if needed |
| 6. Write tests | `request-specs` | Tests pass with new column |

---

## Core Process

1. **[Generate Migration]** — Load skill: `sequel-migrations`
...
2. **[Run Migration]** — Load skill: `cli/db-commands`
   - `hanami db migrate`
   - Verify schema change in database
   - Handoff condition: Schema updated in database

3. **[Update Relation]** — Load skill: `rom-relations`
...
|---|---|
| **sequel-migrations** | Step 1: Create migration. |
| **cli/db-commands** | Step 2: Run migration. |
| **rom-relations** | Step 3: Update Relation schema. |
| **rom-structs-entities** | Step 4: Update Entity attributes. |
| **rom-repositories** | Step 5: Update Repository methods. |
| **request-specs** | Step 6: Update and run tests. |
