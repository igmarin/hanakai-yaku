...
description: >
  ...
  deps-mixin, action-anatomy, and action-unit-specs.
ecosystem_sources:
  - hanami/hanami
  - dry-rb/dry-system
tags:
...
| 2. Inject into Action | `deps-mixin` | Job adapter injectable via `Deps` |
| 3. Enqueue from Action | `action-anatomy` | Action enqueues job correctly |
| 4. Write job specs | `action-unit-specs` | Job class tested in isolation |
...
2. **[Inject into Action]** — Load skill: `deps-mixin`
...
3. **[Enqueue from Action]** — Load skill: `action-anatomy`
   - Define job class
...
4. **[Write Job Specs]** — Load skill: `action-unit-specs`
...
| **deps-mixin** | Step 2: Inject adapter into Actions. |
| **action-anatomy** | Step 3: Enqueue jobs from Actions. |
| **action-unit-specs** | Step 4: Test job classes. |
